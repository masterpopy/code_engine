...
struct template template_indicator = {0x1234, 0x1234, &mega_indicator, (struct frame **)0x82EC69C, 0,
                                        (struct rotscale_frame**) 0x82EC6A8, healthbar_indicator_callback};

struct template template_trigger = {0x2345, 0x2345, &mega_trigger, (struct frame **)0x82EC69C, 0, (struct rotscale_frame**)0x82EC6A8,
                                        healthbar_trigger_callback};
										
struct template template_ztrigger = {0x2346, 0x2345, &mega_trigger, (struct frame **)0x82EC69C, 0, (struct rotscale_frame**)0x82EC6A8,healthbar_trigger_callback};
...
bool not_mega_or_primal(u8 bank)
{
    ...
    else if (!(bank&1) && (!(new_battlestruct->mega_related.party_z_check&bits_table[battle_team_id_by_side[bank]]))&&
