...
bool entry_hazards_hook(void)
{
	...
	else if (ptr_to_struct->stealthrock && !ptr_to_struct->stealthrock_done && not_magicguard(active_bank) && !is_class_FOUR(active_bank))
	{
...
void atk00_move_canceller(void)
{
	...
	if (get_z_moves(current_move, bank_attacker))
	{
		...
		else if (!(new_battlestruct->mega_related.z_happened_pbs & (BIT_GET(bank_attacker) | BIT_GET(bank_attacker ^ 2))))
		{
...
void revert_form_change(bool mega_revert, u8 teamID, u8 side, const struct pokemon* poke)
{
	...
	else
	{
		...
		else
		{
			...
			{0x42b,0x34D},{0x421,0x301},{0x42a,0x34c},{0x422, 0x357},
			{0xFFFF, 0} };
