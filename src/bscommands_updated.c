    ...
    if (weather_abilities_effect() && !(get_item_effect(bank_attacker, 1) == ITEM_EFFECT_SAFETYGOOGLES) &&
        ...
        if (battle_weather.flags.sandstorm || battle_weather.flags.permament_sandstorm) {
            if (!(is_of_type(bank_attacker, TYPE_GROUND) || is_of_type(bank_attacker, TYPE_STEEL) ||
                  is_of_type(bank_attacker, TYPE_ROCK)) && !(ability_effect && (ability == ABILITY_SAND_FORCE ||
...
bool entry_hazards_hook(void) {
    ...
    if (ptr_to_struct->lunardance) {
        ...
        effect = 1;
               ...
               !(side_affecting_halfword[banks_side].spikes_damage_done)
               && GROUNDED(active_bank) && not_magicguard(active_bank) && !is_class_FOUR(active_bank)) {
    ...
    switch (BIC(*move_effect, EFFECT2_AFFECTSUSER)) {
        ...
        case 54: //berry eating
        {
            ...
            if (MOVE_WORKED && !substitute && *berry && get_item_pocket_id(*berry) == 4) {
                ...
                if (arg &&
                    !battle_participants[bank_attacker].current_hp) { break; } //to get the bug bite effect, attacker must be alive
    ...
    while (effect == 0) {
        switch (battle_scripting.cmd49_state_tracker) {
            ...
            case 25: //triple target moves
                if (!(hitmarker & HITMARKER_IMMOBILE_DUE_TO_STATUS) &&
                    move_table[current_move].target == move_target_foes_and_ally &&
...
bool healblock_forbidden_moves(u16 move, u8 with_leechseed) {
    if (move == MOVE_ABSORB || move == MOVE_MEGA_DRAIN || move == MOVE_GIGA_DRAIN || move == MOVE_LEECH_LIFE ||
        move == MOVE_DRAINING_KISS || move == MOVE_DRAIN_PUNCH || move == MOVE_DREAM_EATER || move == MOVE_HORN_LEECH ||
        move == MOVE_OBLIVION_WING || move == MOVE_PARABOLIC_CHARGE || move == MOVE_MILK_DRINK ||
        move == MOVE_HEAL_PULSE || move == MOVE_REST || move == MOVE_RECOVER || move == MOVE_MOONLIGHT ||
        move == MOVE_MORNING_SUN || move == MOVE_SLACK_OFF || move == MOVE_SYNTHESIS || move == MOVE_ROOST ||
...
void atk00_move_canceller(void) {
    ...
    if (is_there_no_target(current_move)) {
        ...
        return;
             ...
             ((hitmarker & (HITMARKER_NO_ATTACKSTRING | 0x800000)) == 0) &&
             new_battlestruct->various.magicbounce == 0) {
        move_outcome.missed = 1;
        ...
        new_battlestruct->various.magicbounce == 0) //magic coat check
    {
        ...
        return;
                ...
                has_ability_effect(bank_target, 1)) &&
               move_table[current_move].move_flags.flags.affected_by_magic_coat &&
    ...
    switch (current_move) {
        ...
        SHAREDWITHASSIST:
            if (move == MOVE_ASSIST || move == MOVE_SLEEP_TALK || move == MOVE_BELCH || move == MOVE_BIDE ||
                ...
                move == MOVE_METRONOME || move == MOVE_MIMIC || move == MOVE_MIRROR_MOVE || move == MOVE_NATURE_POWER ||
                move == MOVE_SKETCH || move == MOVE_STRUGGLE)
                can = 0;
            break;
    }
    return can;
