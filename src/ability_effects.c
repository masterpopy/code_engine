            ...
            for (u8 i = 0; i < 4; i++) {
                ...
                if (move_table[curr_move].base_power > best_power) {
                    ...
                    if (move_table[curr_move].script_id == 70) //one hit ko moves
                        curr_power = 160;
                    else if (curr_move == MOVE_COUNTER || curr_move == MOVE_METAL_BURST ||
                             curr_move == MOVE_MIRROR_COAT)
                        curr_power = 120;
                    else {
                ...
                case ABILITY_MOODY: {
                    ...
                    if (raiseable_stats || lowerable_stats) {
                        ...
                        while (raiseable_stats != 0) {
                            u8 rand_stat = random_value(7)/*__umodsi3(rng(), 7)*/;
        ...
        case 2: //abilities that affect certain moves
            ...
            switch (last_used_ability) {
                ...
                case ABILITY_QUEENLY_MAJESTY:
                    if (get_priority(curr_move, bank_attacker) >= 1 && bank_side == get_bank_side(bank_target) &&
                        bank_side != get_bank_side(bank_attacker)) //JeremyZ
                    {
                        effect = 1;
                ...
                switch (last_used_ability) {
                    ...
                    case ABILITY_SHIELDS_DOWN:
                        if ((battle_participants[bank].species == POKE_MINIOR_CORE ||
                             (battle_participants[bank].species >= 0x3EB &&
                              battle_participants[bank].species <= 0x3F0)) &&
